...
const ModalBand =(data) =>{
    
    const [artistSelected, setArtistSelected] = useState(data);
    const [showList, setShowList] = useState(false);
    const [allBand, setAllBand] = useState()
    const [addIdBand, setIdAddBand] = useState(artistSelected.data.band_id.data.map((id)=> id.id));
    ...
    const deleteBand = (idSelected) =>{
        artistSelected.data.band_id.data.splice(idSelected, 1);
        ...
        if (window.confirm("Voulez-vous supprimer ce groupe?")) { 
            axios.put(`http://localhost:1337/api/artists/${data.data.id}`, {
                data: {
                    band_id: artistSelected.data.band_id.data.map((id)=>id.id)
                }
            } ,{
                headers: {
...
console.log(data);
    return( 
        <div className="center adminUpdate">
            ...
            {data.data.band_id.data && data.data.band_id.data!=0 ?
                <div>
                    <table className="adminTable">
                        ...
                        <tbody>
                            {data.data.band_id.data.map((band, index) =>(
